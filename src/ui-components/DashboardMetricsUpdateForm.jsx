...
import { updateDashboardMetrics } from "../graphql/mutations";
const client = generateClient();
export default function DashboardMetricsUpdateForm(props) {
  const {
    id: idProp,
    dashboardMetrics: dashboardMetricsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = dashboardMetricsRecord
      ? { ...initialValues, ...dashboardMetricsRecord }
      : initialValues;
    setTotalSales(cleanValues.totalSales);
    setTotalExpenses(cleanValues.totalExpenses);
    setTotalProducts(cleanValues.totalProducts);
    setTotalUsers(cleanValues.totalUsers);
    setTotalSellers(cleanValues.totalSellers);
    setTotalBookings(cleanValues.totalBookings);
    setOngoingOrders(cleanValues.ongoingOrders);
    setReturnsCancellations(cleanValues.returnsCancellations);
    setUserRegistrations(cleanValues.userRegistrations);
    setPrimeMembers(cleanValues.primeMembers);
    setSellerRegistrations(cleanValues.sellerRegistrations);
    setCreatedAt(cleanValues.createdAt);
    setUpdatedAt(cleanValues.updatedAt);
    setErrors({});
  };
  const [dashboardMetricsRecord, setDashboardMetricsRecord] = React.useState(
    dashboardMetricsModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getDashboardMetrics.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getDashboardMetrics
        : dashboardMetricsModelProp;
      setDashboardMetricsRecord(record);
    };
    queryData();
  }, [idProp, dashboardMetricsModelProp]);
  React.useEffect(resetStateValues, [dashboardMetricsRecord]);
  const validations = {
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateDashboardMetrics.replaceAll("__typename", ""),
            variables: {
              input: {
                id: dashboardMetricsRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "DashboardMetricsUpdateForm")}
      {...rest}
    >
      <TextField
        label="Total sales"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || dashboardMetricsModelProp)}
