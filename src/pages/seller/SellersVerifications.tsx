...
const Section: React.FC<{ icon: React.ReactNode; title: string; children: React.ReactNode }> = ({ icon, title, children }) => (
  <div className="bg-[#0a0a0a] border border-gray-900 rounded-[2.5rem] p-8 md:p-12 overflow-hidden relative group transition-colors hover:border-gray-800">
    <div className="flex items-center gap-4 mb-10 pb-6 border-b border-gray-900">
      <div className="w-12 h-12 bg-black border border-gray-800 rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform">{icon}</div>
...
const Input: React.FC<{ label: string; value?: string; onChange?: (val: string) => void; placeholder?: string; type?: string; required?: boolean; disabled?: boolean; }> = ({ label, value, onChange, placeholder, type = "text", required = false, disabled = false }) => (
  <div className="space-y-2 group">
    ...
    <input 
      ...
      className={`w-full bg-[#0d0d0d] border border-gray-900 rounded-xl px-4 py-3.5 text-sm transition-all outline-none ${disabled ? 'opacity-50 cursor-not-allowed' : 'focus:border-yellow-500 group-hover:border-gray-700'}`}
    />
  </div>
);
      ...
      <div className="max-w-4xl mx-auto px-6 pt-12">
        ...
        <form onSubmit={handleSubmit} className="space-y-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <div className="w-20 bg-gray-900 border border-gray-800 rounded-xl flex items-center justify-center text-xs font-semibold text-gray-400">
          ...
          <div className="pt-10">
            <button 
              type="submit"
              className="w-full bg-yellow-500 hover:bg-yellow-400 text-black font-semibold py-5 rounded-2xl transition-all shadow-xl shadow-yellow-500/10 flex items-center justify-center gap-3 active:scale-[0.99]"
            >
              Proceed to upload
