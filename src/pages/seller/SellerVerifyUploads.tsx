...
const SellerVerifyUploads: React.FC<SellerVerifyUploadsProps> = ({ 
  ...
  if (submissionId) {
    return (
      <div className="fixed inset-0 bg-black flex flex-col items-center justify-center p-6 z-[10002] text-white font-sans text-center">
    ...
    <div className="min-h-screen bg-black text-white font-sans pb-24">
      <div className="max-w-4xl mx-auto px-6 pt-12">
        <button 
          ...
          className="flex items-center gap-2 text-gray-500 hover:text-white transition-colors mb-12 group disabled:opacity-30"
        >
          ...
          {uploads.map((item) => (
            <div 
              ...
              className={`bg-[#0a0a0a] border-2 rounded-3xl p-8 transition-all relative overflow-hidden flex flex-col items-center text-center justify-center min-h-[220px] ${
                ...
                item.status === 'uploading' ? 'border-yellow-500/40' : 'border-gray-900 hover:border-gray-700 border-dashed'
              }`}
            >
              {item.status === 'idle' && (
                <>
                  <div className="w-16 h-16 bg-gray-900 rounded-2xl flex items-center justify-center text-gray-600 mb-4 group-hover:bg-yellow-500/5 transition-colors">{item.icon}</div>
