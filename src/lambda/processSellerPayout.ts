 ...
 */
async function getSellerFromDatabase(sellerId: string): Promise<any> {
  try {
    const params = {
 ...
 */
async function getSellerOrdersFromDatabase(
  sellerId: string,
  startDate?: string,
  endDate?: string
): Promise<any[]> {
 ...
 */
async function recordPayoutInDatabase(
  sellerId: string,
  payoutData: {
    payoutId: string;
    amount: number;
  ...
  try {
    ...
    const params = {
      ...
      Key: { id: sellerId },
      UpdateExpression:
        'SET last_payout_id = :lpid, last_payout_amount = :lpa, last_payout_date = :lpd, total_payouts = if_not_exists(total_payouts, :zero) + :one, updated_at = :ua',
      ExpressionAttributeValues: {
 ...
 */
export const handler = async (event: AppSyncEvent): Promise<ProcessPayoutResponse> => {
  try {
 ...
 */
export const processSellerPayout = async (input: ProcessPayoutInput): Promise<ProcessPayoutResponse> => {
